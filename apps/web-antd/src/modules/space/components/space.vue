...
// 选中列表
const selection = computed<Eps.SpaceInfoEntity[]>(
  () => refs.inner?.selection || [],
);
  ...
  <div class="cl-upload-space__wrap">
    <slot>
      <template v-if="showBtn">
        ...
        <div
          v-show="urls.length > 0 && showList"
          class="cl-upload-space__wrap-list"
        >
          <cl-upload
            v-model="urls"
            :multiple="multiple"
    ...
    <cl-dialog
      ...
      width="1070px"
    >
      <SpaceInner :ref="setRefs('inner')" v-bind="config" @confirm="confirm" />
