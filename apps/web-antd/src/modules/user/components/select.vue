...
function onSelectionChange(arr: Item[]) {
  // 已加载的
  const ids = new Set(new Set(loadIds.value));
...
async function selectAll() {
  // 全部数据
  await Crud.value?.refresh({ page: 1, size: 10_000 }).then((res) => {
    list.value = res.list;
  });
...
<template>
    ...
    <template v-else>
      <div class="user" @click="open">
        <template v-if="data[0]">
          <cl-avatar :size="24" :src="data[0].avatarUrl" />
  ...
  <cl-dialog v-model="visible" title="选择用户" width="1200px">
    <cl-crud ref="Crud" padding="0">
      <cl-row>
        ...
        <cl-filter label="状态">
          <cl-select :options="options.status" :width="120" prop="status" />
        </cl-filter>
      ...
      <cl-row>
        ...
        >
          <template #column-check="{ scope }">
            <el-button
              v-if="selection[0]?.id == scope.row.id"
              disabled
              type="success"
            >
              已选
