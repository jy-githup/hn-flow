...
// 过滤列表
const list = computed(
  () => session?.list.filter((e) => e.nickName?.includes(keyWord.value)) || [],
);
...
<template>
  <div class="chat-session">
    <div class="head">
      <el-input v-model="keyWord" clearable placeholder="关键字搜索" />
    ...
    <div class="list" v-loading="session?.loading">
      <el-scrollbar :ref="setRefs('scroller')" class="scroller">
        ...
        >
          <div class="avatar">
            <el-badge :hidden="item.num === 0" :value="item.num">
              <cl-avatar :src="item.avatar" shape="square" />
            </el-badge>
          </div>
