...
chartOption.xAxis.data = Array.from({ length: 12 })
  .fill(1)
  .map((e, i) => `${i + 1}月`);
chartOption.series[0].data = Array.from({ length: 12 })
  .fill(1)
  .map(() => Number.parseInt(String(Math.random() * 100)));
chartOption.series[1].data = Array.from({ length: 12 }).fill(100);
</script>
