  ...
  'list|66': [
    {
      ...
      phone() {
        return Mock.Random.integer(13_000_000_000, 19_999_999_999);
      },
    },
  ],
});
...
class Test {
  ...
  async page(params: any) {
    ...
    const list = orderBy(userList, order, sort).filter((e: any) => {
      ...
      if (keyWord !== undefined) {
        f = !!keyWordLikeFields.find((k) =>
          String(e[k]).includes(String(params.keyWord)),
        );
      }

      fieldEq.forEach((k) => {
        if (f && params[k] !== undefined) {
          f = e[k] == params[k];
        }
      });
  ...
  // 更新
  async update(params: { [key: string]: any; id: any }) {
