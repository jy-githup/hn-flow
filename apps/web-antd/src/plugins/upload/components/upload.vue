...
import { computed, nextTick, type PropType, ref, watch, defineOptions } from 'vue';
...
const size = computed(() => {
  ...
  return (isArray(d) ? d : [d, d]).map((e: number | string) =>
    isNumber(e) ? `${e}px` : e,
  );
});
...
async function onBeforeUpload(file: any, item?: Upload.Item) {
  function next() {
    ...
    // 图片预览地址
    if (d.type == 'image' && file instanceof File) {
...
function update() {
  if (!check()) {
    ...
    emit(
      'update:modelValue',
      props.multiple ? getUrls(list.value) : urls[0] || '',
    );
...
<template>
  <div :class="[customClass]" class="cl-upload__wrap">
    <div
      :class="[
        `cl-upload--${type}`,
        {
