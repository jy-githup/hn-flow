...
export default defineComponent({
  ...
  setup(props, { slots }) {
    ...
    async function getHeader(columns: any[], fields: any[]) {
      return columns
        .filter((e) => !e.hidden && fields.includes(e.prop))
        .map((e) => e.label);
    }
    ...
    async function getFileName() {
      return typeof props.filename === 'function'
        ? await props?.filename()
        : props.filename || `报表（${dayjs().format('YYYY-MM-DD HH_mm_ss')}）`;
    }
    ...
    function open() {
      ...
      const columns = orderBy(props.columns, 'orderNum', 'asc').filter(
        (e) =>
          !(
            e.hidden === true ||
            ['expand', 'index', 'op', 'selection'].includes(e.type) ||
            e.filterExport ||
            e['filter-export']
          ),
      );
